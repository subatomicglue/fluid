...
{
    struct FluidCube *cube = (struct FluidCube*) malloc(sizeof(*cube));
    int N = size;
...
void FluidCubeFree(struct FluidCube *cube)
{
    free(cube->s);
    free(cube->density);
...
void FluidCubeAddDensity(struct FluidCube *cube, int x, int y, int z, float amount)
{
    int N = cube->size;
    cube->density[IX(x, y, z)] += amount;
}

void FluidCubeAddVelocity(struct FluidCube *cube, int x, int y, int z, float amountX, float amountY, float amountZ)
{
    int N = cube->size;
    int index = IX(x, y, z);
